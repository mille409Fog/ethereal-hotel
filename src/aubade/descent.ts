/**
 * The lift: five floors, one number, and the rule that turns time into it.
 *
 ...
 * rather than `toBeCloseTo` for that reason — now at five values rather than two,
 * which is five chances for the same invisible failure.
 *
 * Floor −4 adds a sixth consumer of the same exactness, and it is the strictest one
 * yet. `renderer.ts` blends the camera's clock towards the projector's quantised one
 * by `1 − |depth − 4|`, so a settled projection box has to hit 4 exactly or the
 * camera is being evaluated at a fraction of a wall second mixed into a film second
 * — which renders as a picture that judders *almost* right, and is the least
 * diagnosable failure in the piece.
 *
