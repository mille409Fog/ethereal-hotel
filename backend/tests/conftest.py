...
def _seed_fixture(db) -> None:
    ...
    rooms = [
        Room(number="101", room_type=RoomType.STANDARD, floor=1, capacity=3,
             base_rate=100.0, status=RoomStatus.OPERATIONAL),
        Room(number="102", room_type=RoomType.STANDARD, floor=1, capacity=3,
             base_rate=200.0, status=RoomStatus.OPERATIONAL),
        Room(number="103", room_type=RoomType.DELUXE, floor=1, capacity=4,
             base_rate=300.0, status=RoomStatus.OPERATIONAL),
        Room(number="201", room_type=RoomType.SUITE, floor=2, capacity=4,
             base_rate=400.0, status=RoomStatus.MAINTENANCE),
    ]
    db.add_all(rooms)
    ...
    db.add_all(
        [
            ...
            Booking(guest_id=ada.id, room_id=room_a.id,
                    check_in=today - timedelta(days=2), check_out=today + timedelta(days=2),
                    status=BookingStatus.CHECKED_IN, adults=2, children=1, nightly_rate=100.0),
            # B: arrives today and is in house.
            Booking(guest_id=alan.id, room_id=room_b.id,
                    check_in=today, check_out=today + timedelta(days=3),
                    status=BookingStatus.CHECKED_IN, adults=1, children=0, nightly_rate=200.0),
            # C: departs today -> counted as a departure, not in house.
            Booking(guest_id=ada.id, room_id=room_c.id,
                    check_in=today - timedelta(days=3), check_out=today,
                    status=BookingStatus.CHECKED_OUT, adults=2, children=0, nightly_rate=300.0),
            # D: cancelled future booking -> ignored in every metric.
            Booking(guest_id=alan.id, room_id=room_a.id,
                    check_in=today + timedelta(days=5), check_out=today + timedelta(days=7),
                    status=BookingStatus.CANCELLED, adults=1, children=0, nightly_rate=100.0),
        ]
    )
    db.commit()
